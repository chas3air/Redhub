...
func Auth_UsrToProtoUsr(user models.User) (*authv1.User, error) {
...
func Auth_ProtoUsrToUsr(proto_usr *authv1.User) (models.User, error) {
...
func Ums_UsrToProtoUsr(user models.User) (*umv1.User, error) {
	var birthday *timestamppb.Timestamp
	if !user.Birthday.IsZero() {
		birthday = timestamppb.New(user.Birthday)
	}
...
func Ums_ProtoUsrToUsr(proto_usr *umv1.User) (models.User, error) {
	parsedUUID, err := uuid.Parse(proto_usr.GetId())
	if err != nil {
		return models.User{}, err
	}
